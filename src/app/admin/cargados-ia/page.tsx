...
export default function CargadosIAAdminPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-3xl mb-1">Cargados con IA</h1>
      <p className="text-tinta/60 mb-6">
        Solo los resultados que se guardaron usando "Leer con IA y
        autocompletar" en Cargar resultado (uno solo o en lote desde
        una jornada completa). Los cargados a mano siguen viéndose
        únicamente en{" "}
        <span className="font-medium">Historial / Papelera</span>. Eliminar
        o restaurar aquí funciona igual: el ranking y los enfrentamientos
        se recalculan solos.
      </p>
      ...
      {!competicionId && (
        <p className="text-sm text-tinta/50">Elige una competición para ver sus resultados cargados con IA.</p>
      )}
      ...
      {competicionId && !cargando && partidos.length === 0 && (
        <p className="text-sm text-tinta/50">
          Todavía no hay resultados cargados con IA en esta competición.
        </p>
      )}
